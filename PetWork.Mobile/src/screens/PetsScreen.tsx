...
const emptyForm: PetProfileRequest = { name: '', type: 'Kedi', breed: '', age: null, gender: 'Belirtilmedi', description: '', character: '', careNotes: '', childCompatibility: '', otherPetCompatibility: '', isVaccinated: null, isNeutered: null, isMicrochipped: null, tags: [], extraAttributes: {}, isPublic: false };
  ...
  return <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
    <KeyboardAvoidingView style={styles.modalScreen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.sectionLabel}>Profil görünürlüğü</Text>
        <Pressable onPress={() => setForm(current => ({ ...current, isPublic: !current.isPublic }))} style={[styles.visibilityChoice, form.isPublic && styles.visibilityChoiceActive]}><Ionicons name={form.isPublic ? 'eye' : 'eye-off-outline'} size={20} color={form.isPublic ? colors.white : colors.primary} /><View style={styles.flexOne}><Text style={[styles.visibilityTitle, form.isPublic && styles.visibilityTextActive]}>{form.isPublic ? 'Herkese açık' : 'Gizli'}</Text><Text style={[styles.visibilityDescription, form.isPublic && styles.visibilityTextActive]}>{form.isPublic ? 'Pati, kullanıcı profilinde listelenir.' : 'Yalnızca sen ve eşleştiği pati sahibi görebilir.'}</Text></View></Pressable>
...
const styles = createThemedStyles(() => ({
  ...
  visibilityChoice: { minHeight: 72, flexDirection: 'row', alignItems: 'center', gap: 11, paddingHorizontal: 15, borderRadius: 17, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border }, visibilityChoiceActive: { backgroundColor: colors.primary, borderColor: colors.primary }, visibilityTitle: { color: colors.text, fontSize: 12, fontWeight: '900' }, visibilityDescription: { color: colors.muted, fontSize: 9, lineHeight: 14, marginTop: 3 }, visibilityTextActive: { color: colors.white },
